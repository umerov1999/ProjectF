...
comp_func_Plus_one_pixel_const_alpha(uint32_t d, const uint32_t s, const uint32_t const_alpha,
    ...
    return interpolate_pixel(result, const_alpha, d, one_minus_const_alpha);
}

static inline uint32_t comp_func_Plus_one_pixel(uint32_t d, const uint32_t s) {
    const uint32_t result = uint32_t(AMIX(AMASK) | MIX(RMASK) | MIX(GMASK) | MIX(BMASK));
    return result;
}
...
static void color_Source(uint32_t *destPixels, int length, uint32_t color, uint32_t const_alpha) {
    if (const_alpha == 255) {
...
static void
color_SourceOver(uint32_t *destPixels, int length, uint32_t color, uint32_t const_alpha) {
...
//comp_func_Plus_neon
static void src_Source(uint32_t *dst, int length, const uint32_t *src, uint32_t const_alpha) {
    if (const_alpha == 255) {
        uint32_t *const end = dst + length;
        uint32_t *const neonEnd = end - 3;
        ...
        int x = 0;
        const int32_t one_minus_const_alpha = 255 - (int32_t) const_alpha;
        ...
        SIMD_EPILOGUE(x, length, 3) {
            dst[x] = comp_func_Plus_one_pixel_const_alpha(dst[x], src[x], const_alpha,
